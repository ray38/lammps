"""
compute_gmp.py
Purpose: Call `compute_gmp` in LAMMPS and demonstrate extraction of GMP features. 
Serial syntax:
    python compute_gmp.py
Parallel syntax (not implemented yet):
    mpirun -np 4 python compute_gmp.py 
"""
...
def run_lammps(dgradflag):
    ...
    lmp.command(f"compute 	gmp all gmp {snap_options}")
...
# declare compute gmp variables (currently using SNAP settings)
...
lmp_gmp = lmp.numpy.extract_compute("gmp", LMP_STYLE_GLOBAL, LMP_TYPE_ARRAY)

print(lmp_gmp)

# print gmp array to observe
#if (me==0):
...
fref1 = lmp_gmp[0:natoms,0:3].flatten()
eref1 = lmp_gmp[-1,0]
dbdr_length = np.shape(lmp_gmp)[0]-(natoms) - 1
dBdR = lmp_gmp[natoms:(natoms+dbdr_length),3:(nd+3)]
force_indices = lmp_gmp[natoms:(natoms+dbdr_length),0:3].astype(np.int32)

# strip rows with all zero descriptor gradients to demonstrate how to save memory

nonzero_rows = lmp_gmp[natoms:(natoms+dbdr_length),3:(nd+3)] != 0.0
...
# run lammps with dgradflag off, ignore this for now.
"""
if me == 0:
...
lmp_snap = lmp.numpy.extract_compute("gmp", LMP_STYLE_GLOBAL, LMP_TYPE_ARRAY)
natoms = lmp.get_natoms()
