...
ComputeGmp::ComputeGmp(LAMMPS *lmp, int narg, char **arg) :
  ...
  sinnerelem(nullptr), dinnerelem(nullptr), gmptr(nullptr)
{
  ...
  gmptr = new GMP(lmp, rfac0, twojmax, rmin0, switchflag, bzeroflag, chemflag, bnormflag,
...
void ComputeGmp::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
    ...
    if (mask[i] & groupbit) {
      ...
      for (int jj = 0; jj < jnum; jj++) {
        ...
        if (rsq < cutsq[itype][jtype]&&rsq>1e-20) {
          gmptr->rij[ninside][0] = delx;
          gmptr->rij[ninside][1] = dely;
          gmptr->rij[ninside][2] = delz;
          gmptr->inside[ninside] = j;
          gmptr->wj[ninside] = wjelem[jtype];
          gmptr->rcutij[ninside] = (radi+radelem[jtype])*rcutfac;
          if (switchinnerflag) {
            gmptr->sinnerij[ninside] = 0.5*(sinnerelem[itype]+sinnerelem[jtype]);
            gmptr->dinnerij[ninside] = 0.5*(dinnerelem[itype]+dinnerelem[jtype]);
          }
          if (chemflag) gmptr->element[ninside] = jelem;
          ninside++;
        }
      }

      // compute features for atom i

      gmptr->compute_ui(ninside, ielem);
      gmptr->compute_zi();
      gmptr->compute_bi(ielem);

      // loop over neighbors for descriptors derivatives

      for (int jj = 0; jj < ninside; jj++) {
        const int j = gmptr->inside[jj];

        gmptr->compute_duidrj(jj);
        gmptr->compute_dbidrj();
        ...
        if (!dgradflag) {
          ...
          for (int icoeff = 0; icoeff < ncoeff; icoeff++) {
            ...
            snadi[icoeff] += gmptr->dblist[icoeff][0];
            snadi[icoeff+yoffset] += gmptr->dblist[icoeff][1];
            snadi[icoeff+zoffset] += gmptr->dblist[icoeff][2];

            snadj[icoeff] -= gmptr->dblist[icoeff][0];
            snadj[icoeff+yoffset] -= gmptr->dblist[icoeff][1];
            snadj[icoeff+zoffset] -= gmptr->dblist[icoeff][2];
          }
          ...
          if (quadraticflag) {
            ...
            int ncount = 0;
            for (int icoeff = 0; icoeff < ncoeff; icoeff++) {
              double bi = gmptr->blist[icoeff];
              double bix = gmptr->dblist[icoeff][0];
              double biy = gmptr->dblist[icoeff][1];
              double biz = gmptr->dblist[icoeff][2];
              ...
              for (int jcoeff = icoeff+1; jcoeff < ncoeff; jcoeff++) {
                double dbxtmp = bi*gmptr->dblist[jcoeff][0]
                              + bix*gmptr->blist[jcoeff];
                double dbytmp = bi*gmptr->dblist[jcoeff][1]
                              + biy*gmptr->blist[jcoeff];
                double dbztmp = bi*gmptr->dblist[jcoeff][2]
                              + biz*gmptr->blist[jcoeff];
            ...
            for (int icoeff = 0; icoeff < ncoeff; icoeff++) {
              ...
              snap[bik_rows + ((atom->tag[j]-1)*3*natoms) + 3*(atom->tag[i]-1) + 0][icoeff+3] -= gmptr->dblist[icoeff][0];
              snap[bik_rows + ((atom->tag[j]-1)*3*natoms) + 3*(atom->tag[i]-1) + 1][icoeff+3] -= gmptr->dblist[icoeff][1];
              snap[bik_rows + ((atom->tag[j]-1)*3*natoms) + 3*(atom->tag[i]-1) + 2][icoeff+3] -= gmptr->dblist[icoeff][2];

              // dBi/dRi

              snap[bik_rows + ((atom->tag[i]-1)*3*natoms) + 3*(atom->tag[i]-1) + 0][icoeff+3] += gmptr->dblist[icoeff][0];
              snap[bik_rows + ((atom->tag[i]-1)*3*natoms) + 3*(atom->tag[i]-1) + 1][icoeff+3] += gmptr->dblist[icoeff][1];
              snap[bik_rows + ((atom->tag[i]-1)*3*natoms) + 3*(atom->tag[i]-1) + 2][icoeff+3] += gmptr->dblist[icoeff][2];
            }
          }
      } // loop over jj inside
      ...
      if (!dgradflag) {
        ...
        for (int icoeff = 0; icoeff < ncoeff; icoeff++)
          snap[irow][k++] += gmptr->blist[icoeff];

        // quadratic contributions

        if (quadraticflag) {
          for (int icoeff = 0; icoeff < ncoeff; icoeff++) {
            double bveci = gmptr->blist[icoeff];
            snap[irow][k++] += 0.5*bveci*bveci;
            for (int jcoeff = icoeff+1; jcoeff < ncoeff; jcoeff++) {
              double bvecj = gmptr->blist[jcoeff];
...
void ComputeGmp::dbdotr_compute()
{
...
double ComputeGmp::memory_usage()
{
  ...
  bytes += gmptr->memory_usage();                      // SNA object
  int n = atom->ntypes+1;
